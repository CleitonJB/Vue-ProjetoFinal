<template>
  <v-container>
    <v-row class="text-center">
      <v-col cols="12">
           <h1 class="page-header"> Listagem dos carros </h1>
           <v-data-table
              :headers="headers"
              :items="cars"
              :search="search"
    ...
    export default {
        ...
        methods: {
            ...
            async itemAlreadyExist(item) {
                try {
                    const response = await axios.get("http://localhost:3000/cars", { params: { id: item.id } });
            ...
            adicionar(item) {
                this.itemAlreadyExist(item)
                    .then((alreadyExist) => {
                        if(alreadyExist) {
                            alert(`Já existe um carro registrado com o id (${item.id})!`);
                            return;
                        }
                    });
